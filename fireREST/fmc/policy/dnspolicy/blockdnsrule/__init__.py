...
class BlockDnsRule(ChildResource):
    ...
    **Operation IDs:**
    ...
    - `getBlockDNSRule` (GET (list))

    **Query parameters:**

    - `filter` (string, optional): Filter criteria can be specified using the format `name:rulename` `rulename` -- Name of the block rule to be queried
